...
export const RecipesSearch: React.FC<SearchBarProps> = ({
  value = '',
  recipes = [],
  ...
  const placeholderSuggestion = useMemo(() => {
    ...
    for (let i = 0; i < filteredRecipes.length; i++) {
      ...
      if (
        words.some((e) => e.toLowerCase().startsWith(searchValue.toLowerCase()))
      ) {
          ...
          return (
            <div
              className={classNames(
                ...
                { hidden: !searchValue || filteredRecipes.length === 0 },
                `fade-${state}`,
              )}
            >
              {filteredRecipes.length > 0 ? (
                filteredRecipes.map((recipe, index) => (
                  <Link key={index} to={`/recipes/${recipe.slug}`}>
                    <a
