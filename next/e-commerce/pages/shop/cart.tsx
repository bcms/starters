...
const CartPage: React.FC<PageProps<null>> = ({ header, page, footer }) => {
                  ...
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      <Link href={`/shop/${item.slug}`}>
                        <a className="flex text-[24px] leading-none tracking-[-0.48px] mb-3">
                          {item.title}
                        </a>
                      </Link>
...
export const getStaticProps: GetStaticProps<PageProps<null>> = async () => {
  const client = getBcmsClient();
  try {
    const data = await getHeaderAndFooter(client);
    return {
      props: {
        ...data,
        page: null,
