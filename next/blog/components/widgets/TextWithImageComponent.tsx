...
export const TextWithImageComponent: React.FC< {data: TextWithImageWidget}> = ({data}) => {
    ...
    return (
        <div
            className={`flex flex-col gap-6 mb-6 md:mb-8 lg:gap-8 lg:items-start lg:mb-12 ${
                data?.image_position?.selected === 'LEFT' ? 'lg:flex-row-reverse' : 'lg:flex-row'
            }`}
        >
            {hasText && <ContentManager item={data?.text} className="prose" />}
                <BCMSImage
