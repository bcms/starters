  ...
  return (
    <section className="pb-16 lg:pb-[176px]">
      <div className="container">
        <div
          className="leading-none tracking-[-0.02em] font-semibold text-center mb-6 lg:text-5xl lg:leading-none lg:mb-16"
        >
          { data.title }
        </div>
        <div
          className="p-1.5 rounded-[64px] bg-appGray-200 grid grid-cols-4 mb-8 lg:max-w-[828px] lg:mx-auto lg:mb-[128px]"
        >
          {tiers.map((tier) => (
            <button
        ...
        <div className="mb-10 lg:mb-[128px]">
          <Swiper
            {...sliderOptions}
            className="mb-8 lg:mb-[72px]"
          >
            {filteredSponsors.length && filteredSponsors.map((sponsor, index) => (
              <SwiperSlide
                key={index}
              >
                <div
                  className="flex items-center justify-center w-[295px] aspect-[2.17] rounded-lg bg-white mx-auto mb-6 lg:w-[462px] lg:aspect-[1.7] lg:rounded-2xl lg:mb-12"
                >
                  <BCMSImage
                    media={sponsor.cover}
