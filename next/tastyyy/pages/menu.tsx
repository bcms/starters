...
import { MenuPageData, PageProps } from '@/types';
...
const MenuPage: React.FC<PageProps<MenuPageData>> = ({
  header,
  footer,
  page,
}) => {
  ...
  useEffect(() => {
    if (router?.query?.s) {
      setActiveMealType(router.query.s as string);
    }
  }, []);
          ...
          <div className="relative px-4 max-w-[400px] mx-auto lg:max-w-[745px] xl:px-0">
            ...
            <div className="mb-10 lg:mb-20">
              <div className="grid grid-cols-2 gap-x-3 gap-y-4 mb-8 md:flex md:items-center md:justify-center lg:gap-4 lg:mb-10">
                {page.mealTypes.map((mealType, index) => (
                  <button
                    key={index}
                    ...
                    onClick={() =>
                      setActiveMealType(mealType.title.toLowerCase())
                    }
                  >
...
> = async () => {
  ...
  try {
    ...
    const mealTypes = (await client.entry.getAll({
