...
interface PlayerContextValue {
  episode: EpisodeEntryMeta | undefined;
  episodeDOM: HTMLAudioElement | undefined;
  setEpisode: (episode: EpisodeEntryMeta | undefined) => void;
...
export function PlayerProvider(
  ...
  const [episode, setEpisode] = useState<EpisodeEntryMeta | undefined>(
    undefined,
  );
      ...
      if (episodeDOM) {
        ...
        if (obj.playing !== undefined) {
          if (obj.playing) {
            ...
            episodeDOM.addEventListener('timeupdate', updateCurrentTime);
            ...
            episodeDOM.pause();
            episodeDOM.removeEventListener('timeupdate', updateCurrentTime);
          }
        }
      }
    },
  ...
  const handlePrevEpisode = useCallback(() => {
    ...
    if (index > 0 && item) {
      ...
      const audio = new Audio(bcmsMediaToUrl(episodes[index - 1].file));
      audio.preload = 'metadata';
      audio.src = `/api/bcms-media/${episodes[index - 1].file.src}`;
  ...
  const getEpisodeProgressBarWidth = useCallback((): string => {
    return `${
      (settings.currentTime / getPlayingEpisodeFileLength().durationInSeconds) *
      100
    }%`;
