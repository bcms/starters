  ...
  >
    <div class="lg:row-span-2 sticky top-0">
      <div class="grid grid-cols-1 gap-8 border border-appGray-300 p-8">
        ...
        <div>
          <div class="text-2xl leading-none tracking-[-2%] mb-5">Gender</div>
        ...
        <div>
          <div class="text-2xl leading-none tracking-[-2%] mb-5">Price</div>
          <div class="grid grid-cols-1 gap-4">
            <FormCheck
              v-for="(filter, index) in filters.filter(
                (e) => e.type === 'price',
        ...
        <div>
          <div class="text-2xl leading-none tracking-[-2%] mb-5">Brands</div>
          <div class="grid grid-cols-1 gap-4">
            <FormCheck
              v-for="(filter, index) in filters.filter(
                (e) => e.type === 'brand',
    ...
    <div class="lg:col-start-2">
      <div class="flex flex-col gap-4 lg:flex-row lg:gap-8">
        <label class="flex items-center px-5 flex-1 border border-appGray-300">
          <SearchIcon class="w-[18px] h-[18px]" :font-controlled="false" />
          <input
            v-model="searchVal"
            type="search"
            placeholder="Search"
        ...
        <button
          ...
          @click="clearFilters"
        >
          <TrashIcon class="w-[18px] h-[18px]" :font-controlled="false" />
    ...
    <div class="lg:col-start-2">
      ...
      <button
        v-if="loadedProducts < page.products.length"
...
const props = defineProps({
  page: {
    type: Object as PropType<ShopPageData>,
    required: true,
  },
});
...
const filteredProducts = computed(() => {
  return (
    ...
    .sort((a, b) => {
      const priceFilter = activeFilters.value.find((e) => e.type === 'price');
...
onMounted(() => {
  props.page.genders.forEach((gender) => {
    filters.value.push({
      active: false,
      label: gender.title,
      value: gender.slug,
  ...
  props.page.categories.forEach((category) => {
    filters.value.push({
      active: false,
  ...
  props.page.brands.forEach((brand) => {
    filters.value.push({
      active: false,
      label: brand.title,
      value: brand.slug,
