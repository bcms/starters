    ...
    <div class="pt-8 pb-10 overflow-hidden md:pb-20 lg:pt-[72px] lg:pb-[120px]">
      <div class="container">
        ...
        <div class="flex items-center gap-2 lg:gap-6">
          <button
            v-for="(_, index) in data.data.items"
            :key="index"
...
import { APIResponse, TestimonialsPageData } from "~~/types";

const { data } = useAsyncData(async (ctx) => {
  return await ctx?.$bcms.request<APIResponse<TestimonialsPageData>>({
...
const activeItem = computed(() => {
  return data.value?.data.items[activeItemIndex.value];
});
