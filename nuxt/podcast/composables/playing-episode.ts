...
export const usePlayingEpisode = () => {
  const { episodes, bcmsConfig: bcms } = useEpisodes();
  ...
  const episode = useState<EpisodeEntryMetaItem | undefined>(
    'playing-episode',
    () => undefined,
  );
  const setEpisode = (val: EpisodeEntryMetaItem) => {
    episode.value = val;
  };
  ...
  const handlePrevEpisode = () => {
    ...
    if (index > 0 && item) {
      ...
      if(!bcms.value) return;
      const audio = audioUtil.createAudio(bcms.value, episodes.value[index - 1]?.media_file);
